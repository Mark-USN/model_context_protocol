""" 20251010 MMH model_context_protocol.py
    ...
    USAGE: model_context_protocol.py --mode server|client|stop-server [--host HOST] [--port PORT] [--debug True/False]
    Parameters:
        --mode: "server" to start a detached server, "client" to run a client, "stop-server" to stop a detached server.
        --host: Hostname or IP address (default 127.0.0.1)
        --port: TCP port number (default 8085)
        --debug: If True, launch server as child process; if False, launch as detached process (default False)
"""
...
def start_server(host: str, port: int, debug: bool):
    ...
    if debug:
        ...
        """
        Launch the server in a new detached console and allows the parent (this script) to exit immediately.
        The child (server) keeps running and logs to LOG_FILE.
        """
